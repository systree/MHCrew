...
export default function ProfilePage() {
  ...
  const { logout, switchLocation } = useAuth();
  ...
  // Auto-subscribe first-time users (permission never asked yet)
  useEffect(() => {
    if (!pushLoading && pushSupported && !subscribed && permission === 'default') {
      subscribe();
    }
  }, [pushLoading, pushSupported, subscribed, permission, subscribe]);

  const handleSwitchLocation = async (locationId) => {
    setSwitchError('');
    setSwitchingTo(locationId);
    try {
      await switchLocation(locationId);
      // switchLocation updates the auth store — page will re-render with new user/location
    } catch (err) {
      setSwitchError(err?.response?.data?.error ?? 'Failed to switch company. Please try again.');
    } finally {
      setSwitchingTo(null);
    }
  };
  ...
  const activeCompany = locations.find((l) => l.locationId === user?.location_id)?.companyName ?? null;
  ...
  return (
    <div className="page">
      ...
      >
        ...
        {/* Company switcher — only shown when user belongs to multiple locations */}
        {locations.length > 1 && (
          <Section title="Company">
            {switchError && (
              <p style={{ fontSize: 'var(--font-size-sm)', color: 'var(--status-cancelled)', padding: '10px 16px' }}>
                {switchError}
              </p>
            )}
            {locations.map((loc) => {
              const isActive = loc.locationId === user?.location_id;
              return (
                <div
                  key={loc.locationId}
                  style={{
                ...
                >
                  <span style={{ fontSize: 'var(--font-size-sm)', color: isActive ? 'var(--color-text)' : 'var(--color-text-muted)', fontWeight: isActive ? 700 : 400 }}>
                    {loc.companyName}
                  </span>
                  {isActive ? (
                    <span style={{ fontSize: 'var(--font-size-xs)', color: 'var(--color-primary)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em' }}>
                      Active
                    </span>
                  ) : (
                    <button
                      ...
                      onClick={() => handleSwitchLocation(loc.locationId)}
                      style={{
                        padding: '5px 14px',
                        ...
                        fontSize: 'var(--font-size-sm)',
                        fontWeight: 600,
                        cursor: switchingTo !== null ? 'not-allowed' : 'pointer',
                        opacity: switchingTo !== null ? 0.6 : 1,
                      }}
                    >
                      {switchingTo === loc.locationId ? '…' : 'Switch'}
                    </button>
                  )}
                </div>
              );
        ...
        {pushSupported && (
          <Section title="Notifications">
            ...
            >
              ...
              {pushError && (
                <p
                  style={{ fontSize: "var(--font-size-xs)", color: "#f87171" }}
                >
                  {pushError}
                </p>
              )}
        ...
        {/* App info */}
        <Section title="App">
          <ProfileRow icon={infoIcon} label="Version" value="2.0.0" />
